...
//   class_setm.staticmethod("set_mp_default_prec");
	...
	class_<mpq> mpq_inst("q", "Multiple precision rational class", init<>());
	mpq_inst.def("__copy__", &mpq::copy);
	mpq_inst.def("__repr__", &mpq::print_to_string);
	mpq_inst.def(init<const max_fast_int &, const max_fast_int &>());
	mpq_inst.def(init<const max_fast_int &>());
	mpq_inst.def(init<const double &>());
	mpq_inst.def(self += max_fast_int());
	mpq_inst.def(self += double());
	mpq_inst.def(self += self);
	mpq_inst.def(self -= max_fast_int());
	mpq_inst.def(self -= double());
	mpq_inst.def(self -= self);
	mpq_inst.def(self *= max_fast_int());
	...
	mpq_inst.def(self / double());
	mpq_inst.def(self / self);
	mpq_inst.def(max_fast_int() + self);
	mpq_inst.def(double() + self);
	mpq_inst.def(max_fast_int() - self);
	mpq_inst.def(double() - self);
	mpq_inst.def(max_fast_int() * self);
	mpq_inst.def(double() * self);
	mpq_inst.def(max_fast_int() / self);
	mpq_inst.def(double() / self);
	typedef mpq(mpq::*pow_double)(const double &) const;
	typedef mpq(mpq::*pow_int)(const max_fast_int &) const;
	mpq_inst.def("__pow__",pow_double(&mpq::pow));
	mpq_inst.def("__pow__",pow_int(&mpq::pow));
}
