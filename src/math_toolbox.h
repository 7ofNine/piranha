 ...
 */
      Derived pow(const double &power) const
      {
        ...
        if (static_cast<Derived const *>(this)->s_index().begin()->flavour()==false ||
          !(static_cast<Derived const *>(this)->s_index().begin()->trig_args().is_zero()))
        {
...
// Need to think about this, but it is not essential until symbolic coefficients are introduced.
        const cf_type &a=static_cast<Derived const *>(this)->s_index().begin()->c();
