...
namespace piranha
{
	...
	inline RetSeries base_series<__PIRANHA_BASE_SERIES_TP>::sub(const PosTuple &pos_tuple, const SubSeries &s,
		const ArgsTuple &args_tuple) const {
		...
		const const_sorted_iterator it_f = derived_const_cast->template nth_index<0>().end();
		for (const_sorted_iterator it = derived_const_cast->template nth_index<0>().begin(); it != it_f; ++it) {
			RetSeries tmp(it->m_cf.sub<RetSeries>(pos_tuple,s,args_tuple));
			tmp.mult_by(it->m_key.sub<RetSeries>(pos_tuple,s,args_tuple),args_tuple);
